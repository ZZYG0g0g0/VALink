...
def model_test(args, model, tokenizer):
...
def main():
    ...
    codeEncoder = AutoModelForSequenceClassification.from_pretrained(args.code_model_path)
    # BTLink
    ...
    if args.do_train:
        # sample - dataset
        # 一次只读取一个数据集，先下采样，再转化为token，最后用数据去训练
    ...
    if args.do_test:
        ...
        model.to(args.device)
        model_test(args, model, tokenizer)
