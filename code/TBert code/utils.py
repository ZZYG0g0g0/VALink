...
model_output_path = r'../models/TBert_saved_models'  # your saved model path
...
def getargs():
    ...
    parser.add_argument("--tokenizer_name", default=code_model_path, type=str,
...
def convert_examples_to_features(row, tokenizer, args):
    ...
    commit_code = textProcess(row['Commit_Text'], args.key)
    commit_text = textProcess(row['Commit_Code'], args.key)
    issue_token = tokenizer.tokenize(issue_text)
    commit_code_token = tokenizer.tokenize(commit_code)
    commit_text_token = tokenizer.tokenize(commit_text)
    # 如果长度抄了，要对issue和commit进行截断
    if len(issue_token) + len(commit_code_token) + len(commit_text_token) > args.max_seq_length - 3:
        if len(issue_token) > (args.max_seq_length - 3) / 2 and len(commit_text_token) + len(commit_code_token) > (args.max_seq_length - 3) / 2:
            issue_token = issue_token[:int((args.max_seq_length - 3) / 2)]
            if len(commit_code_token) <= (args.max_seq_length - 3) / 4:
                commit_text_token = commit_text_token[:int((args.max_seq_length - 3) / 4 - len(commit_code_token))]
            elif len(commit_text_token) <= (args.max_seq_length - 3) / 4:
                commit_code_token = commit_code_token[:int((args.max_seq_length - 3) / 4 - len(commit_text_token))]
            else:
                commit_text_token = commit_text_token[:int((args.max_seq_length - 3) / 4)]
                commit_code_token = commit_code_token[:int((args.max_seq_length - 3) / 4)]
        elif len(issue_token) > (args.max_seq_length - 3) / 2:
            issue_token = issue_token[:args.max_seq_length - 3 - len(commit_text_token) - len(commit_code_token)]
        elif len(commit_text_token) + len(commit_code_token) > (args.max_seq_length - 3) / 2:
            remain_length = args.max_seq_length - 3 - len(issue_token)
            if len(commit_code_token) <= remain_length / 2:
                commit_text_token = commit_text_token[:int(remain_length - len(commit_code_token))]
            elif len(commit_text_token) <= remain_length / 2:
                commit_code_token = commit_code_token[:int(remain_length - len(commit_text_token))]
            else:
                commit_text_token = commit_text_token[:int(remain_length / 2)]
                commit_code_token = commit_code_token[:int(remain_length / 2)]
    combined_token = [tokenizer.cls_token] + issue_token + [tokenizer.sep_token] + commit_text_token + commit_code_token + [tokenizer.sep_token]
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path=None):
        ...
        for i_row, row in df_link.iterrows():
            self.text_examples.append(convert_examples_to_features(row, tokenizer, args))

    def __len__(self):
        return len(self.text_examples)

    def __getitem__(self, i):
        return (torch.tensor(self.text_examples[i].input_ids), torch.tensor(self.text_examples[i].label),
